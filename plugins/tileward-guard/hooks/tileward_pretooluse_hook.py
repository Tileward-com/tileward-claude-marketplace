...
"""Tileward execution policy as a Claude Code PreToolUse hook. NOT YET IN SERVICE.
    ...
    request shape it sends is settled -- `{tool_name, tool_input, tool_input_fidelity}`, see
    below -- so #190 can be built against it.
...
WHAT IS SENT: the tool NAME **and its INPUT**. Be clear about what that costs, because it is not
small: the input to a Bash call is a command line and the input to an Edit or a Write is your
source code, so this hook sends both to Tileward, and they land in the audit trail.
    ...
    a capability gate instead, with no code change and no fork.
...
Config (environment):
    ...
    TILEWARD_TOOL_INPUT_MAX
                        ...
                        0 sends NO tool_input at all, leaving a capability gate: private, and
                        unable to tell `rm -rf /` from `ls`.
