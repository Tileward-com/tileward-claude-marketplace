...
**This plugin therefore does not register a `PreToolUse` hook.** An earlier version did, with
`matcher: "*"` and the fail-closed default every hook here uses, which meant installing the plugin
refused *every* tool call with "execution policy unreachable (HTTP 404)". The client script
...
it blocks everything, and the only way to stop it blocking is `TILEWARD_FAIL_OPEN=1`, which allows
everything and governs nothing. There is no setting in between, because there is nothing on the
other end.

The request shape **is** settled, as of 2026-08-25: `{tool_name, tool_input}`. The script
originally sent `{"tool_name": "Bash"}` and nothing else, to keep a Bash command line and an Edit's
contents on the machine. But that makes `Bash(rm -rf /)` and `Bash(ls)` byte-identical on the wire,
so it could only ever be a *capability* gate — "may this key run Bash at all" — and never a command
gate. Closing the gap this hook exists for means the input has to leave the machine, and it lands in
the audit trail with the decision. That is a real cost and it is the price of the feature.

Every request declares `tool_input_fidelity`, always present and always one of `full`, `truncated`
or `omitted`, so a policy never has to infer from an absent field whether it saw the whole command.
Long strings are cut at `TILEWARD_TOOL_INPUT_MAX` characters (default 4096); a body still oversize
after that drops the input rather than sending a mangled half, and says so. Setting
`TILEWARD_TOOL_INPUT_MAX=0` restores the old name-only capability gate for anyone unwilling to send
source code — no fork, no code change. #189 and #190 can be built against this shape now.
